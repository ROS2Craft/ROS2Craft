...
public class RobotMenu extends AbstractContainerMenu {
    private ItemStackHandler robotInventory = new ItemStackHandler(3);
    public int entityId = 0; // Stored for client screen
    private final Level level;

    public RobotMenu(int containerId, Inventory playerInv, FriendlyByteBuf extraData) {
        this(containerId, playerInv, extraData.readVarInt());
    }

    public RobotMenu(int containerId, Inventory playerInv, int robotId) {
        super(ModMenuTypes.ROBOT_MENU.get(), containerId);
        this.entityId = robotId;
        this.level = playerInv.player.level();
        this.robotInventory = ((RobotEntity) this.level.getEntity(robotId)).inventory;

        addPlayerInventory(playerInv);
        addPlayerHotbar(playerInv);
        // this.addSlot(new SlotItemHandler(robotInv, 0, 80, 35));
        for (int i = 0; i < 3; i++) {
            this.addSlot(new SlotItemHandler(this.robotInventory, i, 62 + i * 18, 20));
        }
    }
    ...
    public boolean stillValid(Player player) {
        return player.distanceToSqr(level.getEntity(this.entityId)) <= 36;
    }
